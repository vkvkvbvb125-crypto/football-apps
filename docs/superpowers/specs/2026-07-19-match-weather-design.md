...
기상청 단기예보(`getVilageFcst`)는 **발표 시점 기준 약 3일 이내**만 3시간 단위로 상세하게 나온다. 경기 날짜가 오늘부터 3일(72시간) 이내면 예보를 조회하고, 그보다 먼 경기는 엣지함수를 호출하지 않고 클라이언트에서 바로 "아직 예보가 안 나왔어요" 상태로 처리한다(중기예보는 이번 스코프에 포함하지 않음 — 팀원이 경기일에 가까워져서 다시 화면을 열면 자연히 단기예보 범위에 들어와 정상 표시됨).
...
- 중기예보(3~10일) 연동 — 경기일이 3일 이내로 가까워지면 자연히 표시되므로 이번엔 다루지 않는다.
